...
In this article I will show you how to set up an NGINX reverse proxy which will allow you to serve multiple sites on port 80 and 443 (default http/https ports) from a single server.
...
Once the DNS A records have propagated (or you have edited your hosts file) you can visit the domains. Since we haven't yet set up the reverse-proxy, you will also have to specify the port:
...
You could also use any of the other domain name + port combinations. The site is determined strictly based on the port. 
...
For each request we make, the HTTP client (CURL, or the browser) will add a host header which is equal to the domain name that the request is made to. Below is a screenshot of the host header that is set by the browser when we visit red.crewviewer.com.
...
You can run the following commands to start and run NGINX on Debian.
...
Test NGINX on the server via localhost and you should see the default NGINX welcome page.

```sh
curl localhost:80
```

You can also reach NGINX remotely via the public IP address as well as the DNS records that we set up earlier. Since NGINX runs on port 80 by default, and port 80 is the default http port, we don't need to specify the port. This is how you visit most other sites on the web. Since all of the following urls default to port 80, they all return the same default NGINX welcome page.
...
So far all of our requests have used HTTP on port 80. This is fine for some sites, but it means that data moves across the network unencrypted which can allow other people to see your traffic. This is terrible for any security sensitive apps, for obvious reasons.
...
Unfortunately Certbot will only work if you have a domain, it will not work if you used the host file workaround. This is because the domain needs to be in the public DNS and accessible from the Let's Encrypt servers, but the host file change is only on your computer.
:::
...
The CLI tool has multiple options, you can have it install the cert but allow you to manually update the NGINX config file, or you can have it install the cert and automatically update the NGINX config file. We'll walk through both methods in this article.

#### Manually update the NGINX config file

Manually updating the NGINX config file to reference the TLS certificates may help you understand how it is set up. But once you understand what's going on, it is nice to use the automatic command for convenience.

Run the following command on the server. You will be prompted for a few options such as adding an email for upcoming expiration alerts, terms of service agreement, marketing email opt in/out, and selecting the domain that you want to get the certificate for.
...
Once the cert is installed, we will have to reference it from the NGINX configuration file. You can view the certificate and key by running the following commands:
...
Run the following command on the server. If you haven't already run certbot, you will be prompted for a few options such as adding an email for upcoming expiration alerts, terms of service agreement, and marketing email opt in/out. Then you will be prompted to select the domain that you want to get the certificate for.
...
If you go to http://green.crewviewer.com in your browser it should automatically redirect you to the page via HTTPS.
